...
Use the `test.sh` wrapper which automatically cleans up Docker images and volumes after tests:
```
./test.sh -- -f <feature> --skip-autogenerated --skip-duplicated
```

Preserve test artifacts for debugging:
```
./test.sh --preserve -- -f <feature>
```

You can also call `devcontainer features test` directly (but images/volumes won't be cleaned up):
```
devcontainer features test -f <feature>                                    # all tests
devcontainer features test --skip-scenarios -f <feature> -i ubuntu:latest  # autogenerated only
devcontainer features test -f <feature> --skip-autogenerated --skip-duplicated  # scenarios only
devcontainer features test --global-scenarios-only                         # global scenarios
```
...
- Volume names in `mounts` use `${localWorkspaceFolderBasename}-${devcontainerId}` so volumes are identifiable by project name and test volumes (which have numeric temp dir names) are distinguishable from real ones
...
### installsAfter doesn't match local features in tests
`installsAfter` with GHCR references (e.g. `ghcr.io/ekilmer/devcontainer-features/mise`) does **not** match local `./mise` references used by the test framework. To enforce feature install order in test scenarios, use `overrideFeatureInstallOrder` with `./` prefixes in `scenarios.json`.
...
- The test framework cleans up containers but **not** Docker images or volumes. Use `./test.sh` to handle full cleanup.
